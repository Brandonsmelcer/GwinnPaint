...
import '../widgets/exterior_inputs_panel.dart';
import '../widgets/exterior_invoice_panel.dart';
import '../widgets/floor_plan_assistant.dart';
import '../widgets/job_kind_toggle.dart';
...
class _PaintEstimateCalculatorScreenState
  ...
  final _pricePerLinFtController = TextEditingController();
  final _pricePerGallonController = TextEditingController(text: '45');
  final _stainPricePerGallonController = TextEditingController(text: '55');

  EstimateJobKind _jobKind = EstimateJobKind.interior;
  int _coats = 2;
  int _doors = 0;
  int _windows = 0;
  ...
  final List<ExteriorJobLine> _exteriorLines = [
    ExteriorJobLine(id: '0'),
  ];
  int _nextExteriorLineId = 1;
  ...
  void initState() {
    ...
    for (final controller in [
      ...
      _pricePerLinFtController,
      _pricePerGallonController,
      _stainPricePerGallonController,
    ]) {
  ...
  void dispose() {
    ...
    _pricePerLinFtController.dispose();
    _pricePerGallonController.dispose();
    _stainPricePerGallonController.dispose();
    super.dispose();
  }
  ...
  ExteriorEstimateBreakdown get _exteriorBreakdown {
    return ExteriorEstimateBreakdown.calculate(
      jobLines: _exteriorLines,
      laborPricePerSqFt: _parseDouble(_pricePerSqFtController),
      laborPricePerLinFt: _parseDouble(_pricePerLinFtController),
      paintPricePerGallon:
          _parseDouble(_pricePerGallonController).clamp(0, double.infinity),
      stainPricePerGallon:
          _parseDouble(_stainPricePerGallonController).clamp(0, double.infinity),
    );
  }

  void _addExteriorLine() {
    setState(() {
      _exteriorLines.add(ExteriorJobLine(id: '${_nextExteriorLineId++}'));
    });
  }

  void _removeExteriorLine(String id) {
    setState(() {
      _exteriorLines.removeWhere((line) => line.id == id);
      if (_exteriorLines.isEmpty) {
        _exteriorLines.add(ExteriorJobLine(id: '${_nextExteriorLineId++}'));
      }
    });
  }

  void _updateExteriorLine(
    String id,
    ExteriorJobLine Function(ExteriorJobLine) updater,
  ) {
    setState(() {
      final index = _exteriorLines.indexWhere((line) => line.id == id);
      if (index != -1) {
        _exteriorLines[index] = updater(_exteriorLines[index]);
      }
    });
  }
  ...
  Future<void> _textExteriorEstimate() async {
    await EstimateExportService.shareExteriorTextEstimate(_exteriorBreakdown);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
  ...
  Future<void> _downloadExteriorPdf() async {
    await EstimateExportService.downloadExteriorPdf(breakdown: _exteriorBreakdown);
  }

  Future<void> _sendExteriorEstimate() async {
    await EstimateExportService.sendExteriorPdf(breakdown: _exteriorBreakdown);
  }

  EstimateFormSnapshot _buildSnapshot() {
    final isExterior = _jobKind == EstimateJobKind.exterior;
    return EstimateFormSnapshot(
      ...
      totalEstimate:
          isExterior ? _exteriorBreakdown.totalEstimate : _breakdown.totalEstimate,
      blueprintFileName: _blueprintFileName,
      blueprintBase64: _blueprintBase64,
      jobKind: _jobKind,
      stainPricePerGallon: _stainPricePerGallonController.text.trim(),
      laborPricePerLinFt: _pricePerLinFtController.text.trim(),
      exteriorLines: List<ExteriorJobLine>.from(_exteriorLines),
    );
  }
  ...
  void _loadFromRecord(SavedEstimateRecord record) {
    setState(() {
      ...
      _pricePerLinFtController.text = record.laborPricePerLinFt;
      ...
      _jobKind = record.jobKind;
      _stainPricePerGallonController.text = record.stainPricePerGallon;

      _exteriorLines
        ..clear()
        ..addAll(record.toExteriorLines());
      if (_exteriorLines.isEmpty) {
        _exteriorLines.add(ExteriorJobLine(id: '0'));
      }
      _nextExteriorLineId = _exteriorLines.length + 1;
    });
  }
  ...
  Widget _interiorWorkspace({required bool isWide}) {
    final inputs = _InputsPanel(
      ...
      blueprintFileName: _blueprintFileName,
      blueprintBase64: _blueprintBase64,
      onCoatsChanged: (value) => setState(() => _coats = value),
      onDoorsChanged: (value) => setState(() => _doors = value),
      onWindowsChanged: (value) => setState(() => _windows = value),
      ...
      onRoomsChanged: (rooms) => setState(() => _roomDimensions
        ..clear()
        ..addAll(rooms)),
      onBlueprintChanged: (fileName, base64, _) => setState(() {
    ...
    return _splitWorkspace(isWide: isWide, inputs: inputs, invoice: invoice);
  }

  Widget _exteriorWorkspace({required bool isWide}) {
    final inputs = ExteriorInputsPanel(
      laborController: _pricePerSqFtController,
      linFtLaborController: _pricePerLinFtController,
      paintPriceController: _pricePerGallonController,
      stainPriceController: _stainPricePerGallonController,
      lines: _exteriorLines,
      formGeneration: _formGeneration,
      onAddLine: _addExteriorLine,
      onRemoveLine: _removeExteriorLine,
      onUpdateLine: _updateExteriorLine,
    );
    final invoice = ExteriorInvoicePanel(
      breakdown: _exteriorBreakdown,
      onSendEstimate: _sendExteriorEstimate,
      onDownloadPdf: _downloadExteriorPdf,
      onTextEstimate: _textExteriorEstimate,
    );
    return _splitWorkspace(isWide: isWide, inputs: inputs, invoice: invoice);
  }

  Widget _splitWorkspace({
                        ...
                        sliver: SliverToBoxAdapter(
                          child: Column(
                            ...
                            children: [
                              JobKindToggle(
                                value: _jobKind,
                                onChanged: (kind) => setState(() => _jobKind = kind),
                              ),
                              const SizedBox(height: 20),
                              if (isExterior)
                                _exteriorWorkspace(isWide: isWide)
                              else
                                _interiorWorkspace(isWide: isWide),
                            ],
                          ),
                        ),
                      ),
